...
def test_suite9():
    ...
    parties = {
        "Alice": {alice_secret: 6},
        "Bob": {bob_secret: 3},
    }

    expr = ((alice_secret - bob_secret ) + (Scalar(8) - Scalar(4)))
    expected = ((6 - 3) + (8 - 4))
    suite(parties, expr, expected)


def test_custom_app1():
    """
    g1, g2, g3, g4 = [0, ..., 5] (integers only)
    s1, s2, s3, s4 = [0, ..., 5] (integers only)

    f(g1, g2, g3, g4, s1, s2, s3, s4) = (((g1*s1 + g2*s2 + g3*s3 + g4*s4) * Scalar(2)) + Scalar(10))
    """
    g1_secret = Secret()
    g2_secret = Secret()
    g3_secret = Secret()
    ...
    parties = {
        ...
        "grade3": {g3_secret: 5},
        "grade4": {g4_secret: 4},
        "supervisor": {s1_secret: 5,
                        s2_secret: 5,
                        s3_secret: 4,
                        s4_secret: 4}
    }

    expr = (((g1_secret*s1_secret + g2_secret*s2_secret + g3_secret*s3_secret + g4_secret*s4_secret) * Scalar(2)) + Scalar(10))
    expected = (((5*5 + 4*5 + 5*4 + 4*4) * 2) + 10)
    suite(parties, expr, expected)